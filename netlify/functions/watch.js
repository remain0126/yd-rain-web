...
async function dispatchWarningChange(now, prev, event) {
  ...
  // 발표와 발효는 다르다.
  //   발표 — 기상청이 알린 시점. 아직 효력이 없다.
  //   발효 — 실제로 효력이 생기는 시각.
  // 지금 발표만 된 것은 "발표"로 적고 언제 발효되는지 덧붙인다.
  const pendMap = new Map(
    (now.pending || []).filter((p) => p.kind === "발효").map((p) => [p.label, p.at])
  );
  const withWhen = (label) =>
    pendMap.has(label) ? `${label} · ${fmtWhen(pendMap.get(label))} 발효` : label;
  const anyPending = (list) => list.some((l) => pendMap.has(l));
